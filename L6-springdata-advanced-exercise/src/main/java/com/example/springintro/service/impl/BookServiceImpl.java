...
public class BookServiceImpl implements BookService {
    ...
    @Override
    public List<String> findAllBookTitlesWithCopiesLessThan5000() {
    ...
    @Override
    public List<String> findAllBookTitlesWithPriceLesThan5OrMoreThan40() {
    ...
    public List<String> findAllBooksBeforeDate(LocalDate localDate) {
        return bookRepository.findAllByReleaseDateBefore(localDate)
                ...
                .map(book -> String.format("%s %s %.2f",
                        book.getTitle(), book.getEditionType().name(), book.getPrice()))// remember that we have to get name of the enumeration - .name()
                .collect(Collectors.toList());
    }

    @Override
    public List<String> findAllBooksTitleWhereTitleContainsStr(String containingStr) {
        return this.bookRepository.findAllByTitleContainsIgnoreCase(containingStr)
                .stream()
    ...
    @Override
    public List<String> findAllTitlesWithAuthorLastNameStarsWith(String startsWith) {
    ...
    public int findCountOfBooksWithTitleLengthLongerThan(int titleLength) {
        return this.bookRepository.countOfBooksWithTitleLengthMoreThan(titleLength);
    }

    @Override
    public BookSummary getReducedInformationForBookByTitle(String title) {
        return this.bookRepository.findSummaryForTitle(title);
    }

    @Override
    public int addCopiesToBooksAfter(LocalDate localDate, int amount) {
        return this.bookRepository.addCopiesToBooksAfter(localDate, amount);
    }
    ...
    public void changePrice(long bookId) {
        this.bookRepository.changePriceById(bookId);
    }

    @Override
    public int findTotalAmountOfBooksByAuthor(String firstName, String lastName) {
        return this.bookRepository.getTotalCountOfBooksWrittenByAuthor(firstName, lastName);
    }
